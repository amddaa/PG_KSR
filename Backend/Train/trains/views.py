    ...
    if request.method == 'GET':
        ...
        if serializer.is_valid():
            ...
            if not old_departure_time or not old_arrival_time:
                return Response({'error': 'Old schedule details (old_departure_time, old_arrival_time) are required.'},
                                status=status.HTTP_400_BAD_REQUEST)

            service = TrainCommandService()
            if not service.can_update_schedule(train_number, old_departure_time, old_arrival_time, new_departure_time, new_arrival_time):
